...
class CurvedSlider3D {
  ...
  setupDragControls() {
    ...
    this.container.addEventListener("mousedown", (e) => {
      // Kiểm tra xem click có nằm trong vùng cho phép không (30% - 70% của container)
      const rect = this.container.getBoundingClientRect();
      const clickY = e.clientY - rect.top;
      const containerHeight = rect.height;
      const allowedTop = containerHeight * 0.3;
      const allowedBottom = containerHeight * 0.7;
    ...
    this.container.addEventListener(
      ...
      (e) => {
        ...
        // Kiểm tra xem touch có nằm trong vùng cho phép không (30% - 70% của container)
        const rect = this.container.getBoundingClientRect();
